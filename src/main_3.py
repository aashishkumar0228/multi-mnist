...
def remove_zero_padding(arr, y_start_upper=True):
    """
    ...
    new_shape_x = max(1,int(temp_arr.shape[1]*0.8))
    new_shape_y = max(1,int(temp_arr.shape[0]*0.8))
...
def merge(list_file, overlap_prob=True, y_start_upper=True):
    """
    ...
    for (i, arr) in enumerate(list_file):
        arr = remove_zero_padding(arr, y_start_upper)
...
def generator(images, labels, n_digits, n_samples, file_out, name='train', overlap=True):
    ...
    for i in tqdm(range(n_samples), desc='Generate ' + str(n_samples) + ' samples'):
        ...
        im_mer = merge(dig_list, overlap, y_start_upper)
